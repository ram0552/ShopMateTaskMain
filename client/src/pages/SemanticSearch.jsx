        ...
        const fetchResults = async () => {
            ...
            try {
                const response = await axios.get(
                    `${API}/api/products/search/semantic?q=${encodeURIComponent(query)}`
                );
                setResults(response.data);
            } catch (error) {
                console.error(
                    'Error fetching semantic search results:',
                    error
                );
            } finally {
                setLoading(false);
                                ...
                                <div className="p-4">
                                    <div className="flex justify-between items-start">
                                        <div>
                                            ...
                                            >
                                                <h3 className="text-lg font-bold text-gray-900 mb-1 hover:text-gray-600 truncate">
                                                    {product.name}
                                                </h3>
                                            </Link>
                                        </div>
